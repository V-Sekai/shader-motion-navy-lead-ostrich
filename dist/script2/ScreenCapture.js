...
function isFullscreen(win) {
	return win.fullScreen !== undefined ? win.fullScreen // firefox
		: win.outerWidth >= win.screen.width - 16 && win.outerHeight >= win.screen.height - 16; // account for border
		// && (win.innerWidth >= win.screen.width || win.innerHeight >= win.screen.height); // not maximized
}
function isMaximized(win) {
	return win.outerWidth >= win.screen.availWidth && win.outerHeight >= win.screen.availHeight; // account for border
}
function getInnerScreenXY(win) {
	if(win.mozInnerScreenX !== undefined)
		return {x: win.mozInnerScreenX, y: win.mozInnerScreenY};
	if(win[OFFSET] === undefined) { // hack: use mouseevent screenXY
		win[OFFSET] = {x: 0, y: 0};
	...
	const {left,top} = getWindowCapturePadding(win);
	return {x: win[OFFSET].x + (win.screenX-left), y: win[OFFSET].y + (win.screenY-top)};
}
function getWindowCapturePadding(win) {
	if(!/Windows/.test(navigator.userAgent))
		return {left:0, right:0, top:0, bottom:0};
	const border = 8 / win.devicePixelRatio; // windows 8px border
	if(win.mozInnerScreenX !== undefined) // firefox
		return isFullscreen(win) ? {left:0, right:0, top:0, bottom:0}
			: isMaximized(win) ? {left:0, right:-border, top:0, bottom:-border/2} // remove corner
			: {left:0, right:0, top:0, bottom:0};
	else // chrome, edge
		return isFullscreen(win) ? {left:border, right:border, top:border, bottom:border} // add border
			: isMaximized(win) ? {left:0, right:0, top:border, bottom:0} // add padding-top
...
export const getCaptureClientRect = {
	window(win) {
		...
		return {x: win.screenX-left-x, y: win.screenY-top-y, width: win.outerWidth+left+right, height: win.outerHeight+top+bottom};
	},
	monitor(win) {
		const {x, y} = getInnerScreenXY(win);
...
export function getDisplaySurface(track) {
	return track.getSettings().displaySurface || (/monitor/i.test(track.label) ? "monitor" : "window");
}