...
function createTechniques(gl) {
	const gltf = GLTF.loadGLTF({
		...
		images: [
			{bufferView: 0, extras: {type: gl.UNSIGNED_BYTE, internalformat: gl.RGBA, width: 1, height: 1}},
		],
		samplers: [
			{wrapS: gl.CLAMP_TO_EDGE, wrapT: gl.CLAMP_TO_EDGE, magFilter: gl.NEAREST, minFilter: gl.NEAREST},
		],
		textures: [
			{source: 0, sampler: 0},
		],
	...
	const ext = GLTF.loadTechniques({
		shaders: [
			...
			{ type: gl.FRAGMENT_SHADER, _data: Promise.resolve(Shader_MeshPlayer.fs) },
			{ type: gl.FRAGMENT_SHADER, _data: Promise.resolve(Shader_MeshPlayer.fs.replace(/\bdiscard;/, ";")) },
			{ type: gl.VERTEX_SHADER, _data: Promise.resolve(Shader_VideoDecoder.vs) },
			{ type: gl.FRAGMENT_SHADER, _data: Promise.resolve(Shader_VideoDecoder.fs) },
		],
		programs: [
		...
		techniques: [
			...
			{ program: 1, ...techMeshPlayer,
				uniforms: { ...techMeshPlayer.uniforms,
...
function createTextures(gl) {
	...
	const gltf = GLTF.loadGLTF({
		...
		bufferViews: [{buffer: 0}],
		images: [
			{uri: "motion/default.png", extras: {internalformat: gl.RGBA, flipY: true}}, // firefox doesn't support sRGB video texture
			{bufferView: 0, extras: {type: gl.UNSIGNED_BYTE, internalformat: gl.RGBA, width: 40, height: 45}},
		],
		samplers: [
			{wrapS: gl.CLAMP_TO_EDGE, wrapT: gl.CLAMP_TO_EDGE, magFilter: gl.LINEAR, minFilter: gl.LINEAR},
			{wrapS: gl.CLAMP_TO_EDGE, wrapT: gl.CLAMP_TO_EDGE, magFilter: gl.NEAREST, minFilter: gl.NEAREST},
		],
		textures: [
			{source: 0, sampler: 0},
	...
	}, bufferRGBA, location.href);
	const motion = gltf.textures[0];
	const motionDec = gltf.textures[1];
	const framebuffer = gl.createFramebuffer()
	gl.bindFramebuffer(gl.FRAMEBUFFER, framebuffer);
	gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, wGLTF.loadTexture(gl, motionDec), 0);
	return {
		motion: motion,
			...
			readPixels() {
				if(this.needsUpdate) {
					...
					for(let i=0; i<40*45; i++)
						bufferFloat[i] = (((bufferRGBA[i*4+3]/0x100+bufferRGBA[i*4+2])/0x100+bufferRGBA[i*4+1])/0x100+bufferRGBA[i*4])/0x40-1;
				}
				return bufferFloat;
			},
		}),
