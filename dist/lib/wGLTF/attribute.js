...
const numOfComp = {SCALAR: 1, VEC2: 2, VEC3: 3, VEC4: 4, MAT2: 4, MAT3: 9, MAT4: 16};
...
export function bindAttribute(gl, loc, accessor) {
	...
	const {type, componentType, byteOffset=0, normalized=false, bufferView$: {byteStride=0}} = accessor;
	bindBuffer(gl, accessor.bufferView$, gl.ARRAY_BUFFER);
	logGL(gl, "vertexAttribPointer", loc, numOfComp[type], nameGL[componentType], normalized, byteStride, byteOffset);
	gl.vertexAttribPointer(loc, numOfComp[type], componentType, normalized, byteStride, byteOffset);
...
export function bindAttributes(gl, attributes, technique) {
	...
	if(main.hasOwnProperty(_VAO))
		return gl.bindVertexArray(main[_VAO]);
	...
	for(const [name, attr] of Object.entries(technique.attributes)) {
		...
		if(loc !== -1 && acc !== undefined)
			bindAttribute(gl, loc, acc);
	}
}
export async function viewAccessor(accessor) {
	...
	return new TypedArray[componentType](data.buffer, data.byteOffset+byteOffset, count*numOfComp[type]);
}
// primitive: gltf.mesh.primitive
export function drawPrimitive(gl, primitive) {
	const {mode=gl.TRIANGLES, indices$: {count, componentType, byteOffset=0}={}} = primitive;
	if(count === undefined) // no indices
		gl.drawArrays(mode, 0, primitive.attributes$.POSITION.count);
	else if(bindBuffer(gl, primitive.indices$.bufferView$, gl.ELEMENT_ARRAY_BUFFER))
