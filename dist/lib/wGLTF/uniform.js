...
const uniformSetter = {
	[gl.INT]:  "uniform1i", [gl.INT_VEC2]:  "uniform2i", [gl.INT_VEC3]:  "uniform3i", [gl.INT_VEC4]:  "uniform4i",
	[gl.BOOL]: "uniform1i", [gl.BOOL_VEC2]: "uniform2i", [gl.BOOL_VEC3]: "uniform3i", [gl.BOOL_VEC4]: "uniform4i",
	[gl.FLOAT]:"uniform1f", [gl.FLOAT_VEC2]:"uniform2f", [gl.FLOAT_VEC3]:"uniform3f", [gl.FLOAT_VEC4]:"uniform4f",
...
export function bindUniform(gl, loc, uniform, texCounter, call=((o,m,...a)=>o[m](...a))) {
	...
	if(target !== undefined) {
		...
		call(gl, "bindTexture", target, value ? loadTexture(gl, value.index$) : null);
		...
		if(method.startsWith("uniformMatrix"))
			...
			call(gl, method+"v", loc, value);
		else
			call(gl, method, loc, value);
	}
}
export function bindUniforms(gl, values, technique) {
	...
	if(!values.hasOwnProperty(_CMDLIST)) {
		...
		const list = values[_CMDLIST] = [], texCounter = [0];
	...
	for(const [method, args] of values[_CMDLIST])
		gl[method](...args);
}