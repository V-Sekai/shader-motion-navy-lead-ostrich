class AnimationCurve {
	...
	add(time, value) {
		...
		while (n >= 2) {
			const time0 = this.frames[n * 2 - 4], value0 = this.frames[n * 2 - 3];
			const time1 = this.frames[n * 2 - 2], value1 = this.frames[n * 2 - 1];
...
export class AnimRecorder {
	constructor(getHumanPose, frameRate = 30) {
		...
		this.curves = new Map([...RootMotionNames, ...AnimatorMuscleName].map(name => [name, new AnimationCurve()]));
		this.deltaTime = 0;
		this.bucket = 0;
	}
	takeSnapshot(deltaTime) {
		...
		for (let i = 0; i < pose.rootQ.length; i++)
			this.curves.get(`RootQ.${"xyzw"[i]}`).add(this.currentTime, pose.rootQ[i]);
		for (let i = 0; i < pose.muscles.length; i++)
			this.curves.get(AnimatorMuscleName[i]).add(this.currentTime, pose.muscles[i]);
	}
	saveToClip(name = "clip") {
		return new Blob(serializeAnimationClip(this.curves, name), { type: 'text/yaml' });
	}
}