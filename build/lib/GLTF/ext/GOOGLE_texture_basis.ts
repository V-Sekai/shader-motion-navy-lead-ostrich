...
const decoderURLs = Object.fromEntries(
  ["uastc_bc7", "uastc_astc", "uastc_rgba32_srgb"].map((x) => [
    x,
    `https://raw.githubusercontent.com/KhronosGroup/Universal-Texture-Transcoders/main/build/${x}.wasm`,
  ])
);
...
const UnivTexDecoder = {
  load(name) {
    ...
    if (this.decoders[name] === undefined)
      this.decoders[name] = (async (name) =>
        (
          await WebAssembly.instantiate(
            await fetchArrayBuffer(decoderURLs[name]),
  ...
  async decodeRGBA32(name, width, height, compressedData) {
    ...
    return (func) => {
      ...
      const buffer = this.realloc(
        (inputLength + width * yBlocks * 16 + 65535) >> 16
      );
...
imageLoaders["image/basis"] = (data, img) => (gl) => {
  const _data$$ = data.then((basis) => {
    ...
    const width = slice.getUint16(5 /*m_orig_width*/, true),
      height = slice.getUint16(7 /*m_orig_height*/, true);
    const file = basis.subarray(
      slice.getUint32(13 /*m_file_ofs*/, true),
      slice.getUint32(17 /*m_file_size*/, true)
    );

    img.extras = { width: width, height: height };
    let ext;
    if ((ext = gl && gl.getExtension("EXT_texture_compression_bptc"))) {
      Object.assign(img.extras, {
        compressed: true,
        format: null,
        type: null,
    ...
    return UnivTexDecoder.decodeRGBA32(
      "uastc_rgba32_srgb",
      width,
      height,
      file
