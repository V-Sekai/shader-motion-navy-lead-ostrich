...
export class OrbitControls {
	...
	constructor(camera, canvas) {
		...
		function handleOrbit(movementX, movementY) {
			...
			scope.rotationXY[0] = Math.min(Math.max(scope.rotationXY[0], -Math.PI/2), +Math.PI/2);
		}
		function handlePanZoom(movementX, movementY, scale=1, clientX, clientY) {
			const offsetX = clientX === undefined ? 0 : clientX - (canvas.clientLeft + canvas.clientWidth/2);
			const offsetY = clientY === undefined ? 0 : clientY - (canvas.clientTop + canvas.clientHeight/2);
			const clientDist = canvas.clientHeight/2/Math.tan(camera.yfov/2);
			scope.translation[2] /= scale;
			scope.translation[0] -= scope.translation[2]/clientDist * (movementX-offsetX*(scale-1));
			scope.translation[1] += scope.translation[2]/clientDist * (movementY-offsetY*(scale-1));
		}
		...
		function onPointerDown(event) {
			if(!scope.enabled || (event.pointerType !== "mouse" && event.pointerType !== "pen"))
				return;
			event.preventDefault();
		...
		function onPointerMove(event) {
			if(!scope.enabled || (event.pointerType !== "mouse" && event.pointerType !== "pen"))
				return;
			event.preventDefault();
			if(scope.buttonOrbit !== undefined && (event.buttons & 1<<scope.buttonOrbit))
				handleOrbit(event.movementX, event.movementY);
			if(scope.buttonPan !== undefined && (event.buttons & 1<<scope.buttonPan))
		...
		function onTouchMove(event) {
			...
			const touch0 = touch;
			touch = getAverageTouch(event.touches);
			if(touch.length !== touch0.length)
				return;
			const movementX = touch.clientX - touch0.clientX;
			const movementY = touch.clientY - touch0.clientY;
			if(touch.length === 1)
				handleOrbit(movementX, movementY);
			else
				handlePanZoom(movementX, movementY, touch.scale/touch0.scale, touch0.clientX, touch0.clientY);
		}
...
function getAverageTouch(touches) {
	...
	return {clientX: sumX/n, clientY: sumY/n, length: n, scale: Math.sqrt((sumX2*n-sumX**2) + (sumY2*n-sumY**2))/n};
}