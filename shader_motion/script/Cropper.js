export class Cropper {
	...
	resize(rect) {
		...
		this.elem.style.left = rect.left - par.left + this.rect[0] * rect.width + "px";
		this.elem.style.top = rect.top - par.top + this.rect[1] * rect.height + "px";
	...
	crop(clientX, clientY) {
		...
		const bbox = [rect.left, rect.top, rect.right, rect.bottom];
		this.rect[i] = Math.max(0, Math.min(1, ([clientX, clientY][i % 2] - bbox[i % 2]) / (bbox[i % 2 + 2] - bbox[i % 2])));
		this.resize(rect);
	}
}
