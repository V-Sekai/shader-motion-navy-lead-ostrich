...
export class AnimRecorder {
	...
	SaveToClip(name="clip") {
		const curves = [...this.curves].map(([name, curve]) => ["", "Animator", name, curve]);
		return new Blob(SerializeAnimationClip({
			name: name, curves: curves, frameRate: this.frameRate}), {type : "text/yaml"});
	}
}