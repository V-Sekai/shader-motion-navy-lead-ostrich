...
const _SHADER = Symbol("GLShader"), _PROGRAM = Symbol("GLProgram"), _READY = Symbol("ready");
...
export function loadShader(gl, shader) {
	if(shader.hasOwnProperty(_SHADER))
...
export function loadProgram(gl, program) {
	if(program.hasOwnProperty(_PROGRAM))
		return program[_PROGRAM];
	const shaders = [program.vertexShader$, program.fragmentShader$];
	const prog = gl.createProgram();
	Promise.all(shaders.map(s => (gl.attachShader(prog, loadShader(gl, s)), s[_READY]))).then(_ => {
		gl.linkProgram(prog);
		program[_READY] = null;
	});
...
export function useProgram(gl, program) {
	...
	if(program[_READY] === null && !(program[_READY] = gl.getProgramParameter(prog, gl.LINK_STATUS))) {
		const logs = gl.getAttachedShaders(prog).map(shader => gl.getShaderInfoLog(shader));
